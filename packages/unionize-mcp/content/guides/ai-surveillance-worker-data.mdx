---
title: "AI Surveillance and Worker Data"
slug: "ai-surveillance-worker-data"
category: "AI, Surveillance & Worker Data"
page_type: "playbook"
jurisdiction: "US-private-sector"
legal_scope: "Educational, not legal advice"
last_reviewed: "2026-08-01"
review_status: "needs-labor-lawyer-review"
risk_level: "high"
source_status: "mixed"
when_to_use: "Use when management expands monitoring, dashboards, or AI-readiness demands and workers need the broader frame."
not_for: "Not for technical threat modeling or a step-by-step response to one specific keystroke tool."
collections:
  - "issue-guides"
related_slugs:
  - "keystroke-tracking-ai-training"
  - "what-to-preserve-checklist"
  - "protected-concerted-activity"
sources:
  - title: "GC 25-05 Rescission of Certain General Counsel Memoranda"
    url: "https://www.nlrb.gov/news-outreach/news-story/gc-25-05-rescission-of-certain-general-counsel-memoranda"
    publisher: "National Labor Relations Board"
    kind: "government"
    note: "Official notice that the NLRB Acting General Counsel rescinded GC Memo 23-02 on electronic monitoring on February 14, 2025."
  - title: "Artificial Intelligence and the ADA"
    url: "https://www.eeoc.gov/eeoc-disability-related-resources/artificial-intelligence-and-ada"
    publisher: "U.S. Equal Employment Opportunity Commission"
    kind: "government"
    note: "EEOC resource hub on AI, software, algorithms, and disability-related employment protections."
  - title: "Concerted Activity"
    url: "https://www.nlrb.gov/about-nlrb/rights-we-protect/the-law/employees/concerted-activity"
    publisher: "National Labor Relations Board"
    kind: "government"
    note: "Current NLRB explanation of covered employees' right to act with coworkers about work-related issues, subject to coverage and conduct limits."
---

If your employer suddenly wants more telemetry, more dashboards, more screenshots, or more "AI readiness," workers are usually right to pay attention. A system that is sold as efficiency or quality control often becomes a way to tighten discipline, extract data, and change working conditions without real worker input.

## What this guide is for

Use this page when workers are being monitored, scored, ranked, or mined for data in ways that feel larger than one manager's bad habit. The point is not to panic. The point is to recognize when a technical rollout is also a power shift at work.

## Common patterns

- Keystroke and mouse tracking sold as "productivity visibility"
- Screenshot capture, screen recording, or app-usage logging
- AI review systems that score workers without meaningful explanation
- Data collection framed as training an internal assistant, copilot, or replacement workflow
- New metrics that quietly change how workers are judged, compared, or coached

## Why workers read this as a workplace issue

Surveillance changes more than privacy. It can change pace, pressure, discipline, promotion decisions, and who feels safe speaking honestly at work.

When many workers are affected at once, the problem is usually bigger than an individual complaint. It becomes a question of workload, data use, power, and whether management gets to unilaterally rewrite the terms of the job.

## Keep the legal footing narrow

There is no single federal rule that makes ordinary workplace monitoring unlawful merely because it is invasive. Coverage can depend on what the system collects, how the employer uses it, which workers are affected, and federal, state, or local law.

Two narrower points have current agency support. Covered employees may have NLRA protection when they [act together about monitoring or other working conditions](https://www.nlrb.gov/about-nlrb/rights-we-protect/the-law/employees/concerted-activity). Employers also remain responsible for compliance with federal anti-discrimination law when software or AI affects hiring or employment decisions; the [EEOC's AI and ADA resources](https://www.eeoc.gov/eeoc-disability-related-resources/artificial-intelligence-and-ada) explain that boundary.

An earlier NLRB General Counsel memo proposed a broader framework for electronic surveillance, but the agency [rescinded that memo on February 14, 2025](https://www.nlrb.gov/news-outreach/news-story/gc-25-05-rescission-of-certain-general-counsel-memoranda). This guide therefore does not present that memo as current law or current enforcement policy.

## What to preserve

- Rollout announcements, policy updates, and manager guidance
- The names of the tools involved
- Any explanation of what is being collected and why
- Dates, teams, and roles affected
- Evidence that the data is tied to discipline, ratings, layoffs, or AI training

Keep notes on personal systems. Preserve only what you are lawfully allowed to keep.

## Questions to compare with coworkers

- Is everyone getting the same explanation?
- Is the monitoring new, or is management only being more explicit about it now?
- Is the data tied to performance reviews, coaching, or layoffs?
- Are some teams carrying more of the surveillance than others?
- Are workers being told this is for AI, compliance, productivity, or all three at once?

## Safer first steps

1. Compare the facts with one or two trusted coworkers off company systems.
2. Figure out whether the issue is isolated, team-wide, or company-wide.
3. Move from private anger toward shared questions and shared demands.
4. Keep the early focus on clarity, not bravado.

## What not to do

Do not sabotage systems, falsify work, corrupt data, or use company systems to plan organizing activity.

## Related guides

- [My Employer Is Tracking Computer Activity for AI Training](/resources/keystroke-tracking-ai-training)
- [Protected Concerted Activity](/resources/protected-concerted-activity)
- [Retaliation Warning Signs](/resources/retaliation-warning-signs)
